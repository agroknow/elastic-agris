...
FACET_LABELS['pub_year'] = 'Publication Year';
FACET_LABELS['language'] = 'Language';
FACET_LABELS['lrt'] = 'Resource Type';
FACET_LABELS['collection'] = 'Collection';
FACET_LABELS['manifestations'] = 'Manifestation Types';
/*--end mapping*/
...
//--->there is a js check to avoid this
//if($F('query').blank)
//request = "{\"query\" : {\"query_string\" : {\"query\" : \"*\" }},"+filteringLRT+"\"facets\" : {\"lrt\": {\"terms\": {\"field\": \"title\",\"all_terms\": true}}}}";
                        
                        
                        //REQUEST
                        if(!$F('query').blank())
                        request = "{ \"from\" : "+start+", \"size\" : "+PAGE_SIZE+", \"query\" : {\"query_string\" : {\"query\" :\""+$F('query')+"\" }},"+filter+"\"facets\" : {  \"lrt\": {\"terms\": {\"field\": \"value\",\"all_terms\": true}},   \"collection\": {\"terms\": {\"field\": \"set\",\"all_terms\": true}},  \"pub_year\": {\"terms\": {\"field\": \"date\",\"all_terms\": true}},   \"language\": {\"terms\": {\"field\": \"language\",\"all_terms\": true}},   \"manifestations\": {\"terms\": {\"field\": \"manifestationType\",\"all_terms\": true}}}}";
...
jQuery.ajax({
       //url: "http://83.212.100.142:9200/agris/_search",
       url: "http://83.212.100.142:9200/agris/_search",
       type: 'POST',
       dataType: "json",
       data:request,
 ...
 elasticResponse.hits.hits.each(function(item,index){
                      oddCtr++;
                                ...
                                //alert(tmp.expressions[0].manifestations[1].format[0] + ' ----- ' +tmp.expressions[0].manifestations[1].items[0].url + ' ---- ' );
                        
                    
                      if (tmp.expressions[0].manifestations[1].format[0].indexOf('pdf') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/pdf.png';
                      else if (tmp.expressions[0].manifestations[1].format[0].indexOf('powerpoint') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/ppt.png';
                      else if (tmp.expressions[0].manifestations[1].format[0].indexOf('video') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/video.png';
                      else if (tmp.expressions[0].manifestations[1].format[0].indexOf('zip') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/zip.png';
                      else if (tmp.expressions[0].manifestations[1].format[0].indexOf('audio') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/audio.png';
                      else if ((tmp.expressions[0].manifestations[1].format[0].indexOf('text') != -1) ||(tmp.expressions[0].manifestations[1].format[0].indexOf('multipart') != -1) )
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/url.png';
                      //tmp.expressions[0].manifestations[1].format[0]='http://open.thumbshots.org/image.aspx?url='+'tmp.location';
                      else if ((tmp.expressions[0].manifestations[1].format[0].indexOf('xml') != -1 || tmp.expressions[0].manifestations[1].format[0].indexOf('XML') != -1) )
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/xml.png';
                      else if (tmp.expressions[0].manifestations[1].format[0].indexOf('image') != -1)
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/image.png';
                      else if ((tmp.expressions[0].manifestations[1].format[0].indexOf('word')!= -1) || (tmp.expressions[0].manifestations[1].format[0].indexOf('wordprocessingml')!= -1))
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/word.png';
                      else if ((tmp.expressions[0].manifestations[1].format[0].indexOf('application')!= -1))
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/application.png';
                      else if ((tmp.expressions[0].manifestations[1].format[0] == ''))
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/application.png';
                      else
                      tmp.expressions[0].manifestations[1].format[0]='images/icons/url.png';
 ...
 function initializeJamlTemplates()
{
 ...
 Jaml.register('result', function(data){
               ...
               article({class:'item-intro '+odd},
                       header(
                              h2(img({src:data.expressions[0].manifestations[1].format[0]}),
                                 a({href:data.location,title: data.languageBlocks[thisLanguage].title, target: '_blank'},data.languageBlocks[thisLanguage].title)),
